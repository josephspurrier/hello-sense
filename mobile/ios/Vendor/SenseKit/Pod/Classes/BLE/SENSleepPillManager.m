...
//
//
// iOSDFULibrary 4.x renamed its module to NordicDFU.
@import NordicDFU;
...
- (void)beginDfuWithLocalURL:(NSURL*)localURL {
    ...
    // The initialiser began throwing in 4.x rather than returning nil, so an
    // unreadable or malformed binary now surfaces here instead of failing later.
    ...
    DFUFirmware* firmware = [[DFUFirmware alloc] initWithUrlToBinOrHexFile:localURL
                                                              urlToDatFile:nil
                                                                     ...
                                                                     error:&firmwareError];
    if (!firmware) {
        DDLogVerbose(@"failed to load firmware binary %@", firmwareError);
        NSError* error = [[self class] errorWithCode:SENSleepPillErrorCodeDfuError
                                              reason:@"could not read firmware binary"];
