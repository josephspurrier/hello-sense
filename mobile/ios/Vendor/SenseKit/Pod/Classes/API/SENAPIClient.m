...
// AFNetworking 4.0 inserts a per-request headers: argument after parameters: on
// each of these. Passing nil keeps the previous behaviour, where every header is
// set on the shared request serializer by setValue:forHTTPHeaderField: above.

+ (void)GET:(NSString *)URLString parameters:(id)parameters completion:(SENAPIDataBlock)completion {
    [[self HTTPSessionManager] GET:[self urlEncode:URLString] parameters:parameters headers:nil progress:nil
...
{
    [[self HTTPSessionManager] POST:[self urlEncode:URLString] parameters:parameters headers:nil progress:nil
                            success:SENAPIClientRequestSuccessBlock(completion)
                            failure:SENAPIClientRequestFailureBlock(completion)];
}
...
{
    [[self HTTPSessionManager] PUT:[self urlEncode:URLString] parameters:parameters headers:nil
                           success:SENAPIClientRequestSuccessBlock(completion)
                           failure:SENAPIClientRequestFailureBlock(completion)];
}
...
{
    [[self HTTPSessionManager] DELETE:[self urlEncode:URLString] parameters:parameters headers:nil
                              success:SENAPIClientRequestSuccessBlock(completion)
                              failure:SENAPIClientRequestFailureBlock(completion)];
}
...
{
    [[self HTTPSessionManager] PATCH:[self urlEncode:URLString] parameters:parameters headers:nil
                             success:SENAPIClientRequestSuccessBlock(completion)
                             failure:SENAPIClientRequestFailureBlock(completion)];
}
