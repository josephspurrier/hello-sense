...
class SlidingNavigationTitleView: UIView {
    ...
    fileprivate func addControl(title: String!, index: Int, total: Int) {
        ...
        let control = UIButton(type: UIButton.ButtonType.custom)
        
        control.setTitle(title, for: UIControl.State.normal)
    ...
    @objc func tap(control: UIButton) {
        let title = control.title(for: UIControl.State.normal)!
