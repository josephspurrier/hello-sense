...
@available(iOS 8.2, *)
@objcMembers class SenseStyle: NSObject {
    
    static let themeKey = "is.hello.app.theme"
    static let theme = Theme()
    static var currentTheme: SupportedTheme?

    /// Whether the night theme is active. Exposed because system-drawn chrome
    /// has to be told which appearance to match; it does not read this theme.
    @objc static var isNightTheme: Bool {
        return currentTheme == .night
    }

    /// Re-applies the loaded theme to the current view hierarchy. The theme is
    /// first loaded before the window exists, so that pass has nothing to walk;
    /// call this once the window and its root controller are in place.
    @objc static func reapplyTheme() {
        theme.apply(auto: true)
    }
    ...
    @objc static func saveTheme(theme: SupportedTheme, auto: Bool) {
        let preferences = SENLocalPreferences.shared()!
        // Must be rawValue: loadSavedTheme reads this back through
        // SupportedTheme(rawValue:). These happened to agree under Swift 3, but
        // since Swift 4.2 hashValue is randomly seeded per process, so the
        // stored number never matched a case again and the saved theme silently
        // fell back to day.
        preferences.setUserPreference(theme.rawValue, forKey: themeKey)
