...
class SlideContentPresenter: HEMPresenter {
    ...
    fileprivate func title(controller: UIViewController!) -> String! {
        ...
        if controller.title != nil  {
            ...
            // object_getClass now returns AnyClass? rather than AnyClass.
            controllerTitle = object_getClass(controller).map { NSStringFromClass($0) }
        }
...
extension SlideContentPresenter {
    ...
    @objc func bind(activity: HEMActivityIndicatorView?) {
        activity?.stop()
    ...
    @objc func bind(navigationBar: UINavigationBar?) {
    ...
    @objc func bind(scrollView: UIScrollView!) {
...
extension SlideContentPresenter: SlidingNavigationDelegate {
    ...
    @objc func didTapOn(title: String, from: SlidingNavigationTitleView) {
