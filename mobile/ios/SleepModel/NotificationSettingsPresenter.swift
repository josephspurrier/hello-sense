...
@objcMembers class NotificationSettingsPresenter: HEMPresenter {
    ...
    @objc init(service: PushNotificationService!) {
        super.init()
    ...
    fileprivate func reloadTableHeader() {
        if UIApplication.shared.canSendNotifications() {
            ...
            warningView.frame.size = warningView.systemLayoutSizeFitting(UIView.layoutFittingExpandedSize)
