...
@objcMembers class TabPresenter: HEMPresenter {
    ...
    fileprivate func reloadIcons() {
        if self.iconHighlighted != nil {
            self.iconHighlighted = self.iconHighlighted!.withRenderingMode(UIImage.RenderingMode.alwaysOriginal)
        }
        
        if self.icon != nil {
            self.icon = self.icon!.withRenderingMode(UIImage.RenderingMode.alwaysOriginal)
        }
