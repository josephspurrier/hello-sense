...
- (void)loginFrom:(id)controller completion:(HEMFacebookLoginHandler)completion {
    ...
    // The read/publish permission split was removed; there is one method now.
    [[self loginManager] logInWithPermissions:[self profilePermissions]
    ...
    [self loginFrom:controller completion:^(BOOL cancelled, NSError * _Nullable error) {
        ...
        if (error) {
            ...
            FBSDKGraphRequest* request = [[FBSDKGraphRequest alloc] initWithGraphPath:@"me"
                                                                           parameters:params];
            // The connection argument is now the FBSDKGraphRequestConnecting protocol.
