...
extension SlideContainerViewController: SlideContentVisibilityDelegate {
    ...
    @objc func addController(controller: UIViewController, from _: SlideContentPresenter?) {
        self.addChild(controller)
        controller.didMove(toParent: self)
    }
    
    @objc func removeController(controller: UIViewController, from _: SlideContentPresenter?) {
        controller.willMove(toParent: nil)
        controller.removeFromParent()
    }
    
    @objc func update(viewAtIndex: Int, visible: Bool, from _: SlideContentPresenter?) {
        if viewAtIndex < self.children.count {
            let controller = self.children[viewAtIndex]
