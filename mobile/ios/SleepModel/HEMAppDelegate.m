...
- (void)configureCrashReport {
    ...
    // Debug ships an empty token deliberately, to keep crash reporting off. An
    // empty NSString is still truthy, and Bugsnag 6 throws on an empty API key
    // where Bugsnag 4 quietly accepted it, so the length has to be checked.
    if ([token length] > 0) {
        // Bugsnag 6 builds the configuration up front and starts with it,
        // rather than starting first and mutating a shared configuration.
        BugsnagConfiguration* bugsnagConfig = [[BugsnagConfiguration alloc] initWithApiKey:token];
        ...
        [Bugsnag startWithConfiguration:bugsnagConfig];

        NSString* accountId = [SENAuthorizationService accountIdOfAuthorizedUser];
        if (accountId) {
            // setUser moved onto Bugsnag itself and swapped name/email order.
            [Bugsnag setUser:accountId withEmail:nil andName:nil];
        }
...
- (void)createAndShowWindow {
    ...
    self.window = window;
    // Without this the window is black, which shows wherever a controller's
    // view does not reach, such as behind the floating tab bar. The theme also
    // sets this when it is applied, but the window does not exist yet the first
    // time that runs.
    self.window.backgroundColor = [SenseStyle colorWithAClass:[UIViewController class]
                                                     property:ThemePropertyBackgroundColor];
    // loadTheme runs before this, so the theme could not reach the window on
    // launch; it keeps both of these in step on every later theme change.
    self.window.overrideUserInterfaceStyle = [SenseStyle isNightTheme]
                                           ? UIUserInterfaceStyleDark
                                           : UIUserInterfaceStyleLight;
    [self.window makeKeyWindow];
    self.window.rootViewController = [HEMMainStoryboard instantiateRootViewController];
    [self.window makeKeyAndVisible];
    // loadTheme ran before any of this existed, so its pass over the view
    // hierarchy had nothing to walk. Apply it again now that it does.
    [SenseStyle reapplyTheme];
}
