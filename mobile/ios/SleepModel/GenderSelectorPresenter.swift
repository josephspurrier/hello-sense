...
@objcMembers class GenderSelectorPresenter: HEMPresenter {
    ...
    @objc init(account: SENAccount!, onboardingService: HEMOnboardingService!) {
    ...
    @objc func bind(skipButton: UIButton?) {
        ...
        skipButton?.setTitleColor(UIColor.grey3(), for: UIControl.State.disabled)
        skipButton?.titleLabel?.font = UIFont.button()
        skipButton?.setTitle(title, for: UIControl.State.normal)
    ...
    @objc func bind(nextButton: UIButton?) {
        ...
        nextButton?.setTitle(title.uppercased(), for: UIControl.State.normal)
...
extension GenderSelectorPresenter: HEMListDelegate {
    ...
    @objc func goBack(from presenter: HEMListPresenter) {
        self.updateDelegate?.dismissOtherOptionsFrom(from: self)
    }
    
    @objc func didSelectItem(_ item: Any, at index: Int, from presenter: HEMListPresenter) {
