...
@objcMembers class Theme: NSObject {
    ...
    fileprivate func apply(viewController: UIViewController?, auto: Bool) {
        ...
        if let themedVC = controller as? Themed {
            themedVC.didChange(theme: self, auto: auto)
        } else if controller.isViewLoaded {
            // Containers such as UINavigationController and the slide
            // containers behind the middle tabs are not Themed, so their own
            // view never got a background and whatever it defaulted to showed
            // through wherever the child does not cover, like behind the tab bar.
            controller.applyStyle()
        }

        // This walk used to stop at anything that was not Themed, so a
        // non-Themed container also hid every controller inside it from the
        // theme. Those only looked right because switching tabs rebuilds them.
        controller.children.forEach { (child: UIViewController) in
...
fileprivate extension Theme {
    ...
    fileprivate func applyApearances() {
        ...
        // The window's background was never set, so it defaulted to black. That
        // only shows where a controller's view does not cover the screen, which
        // on modern iOS is the area behind the floating tab bar: the timeline
        // container does not extend that far, so black showed through there
        // while the other tabs painted their own background.
        let windowBackground = self.value(aClass: UIViewController.self, property: .backgroundColor) as? UIColor

        // System-drawn chrome, notably the floating tab bar's material, follows
        // the trait environment rather than this theme, so it stayed light in
        // night mode and picked up whichever screen it sat over. Only a custom
    ...
    fileprivate func applyTabBarAppearance() {
        ...
        let barTint = self.value(aClass: UITabBar.self, property: .barTintColor) as? UIColor
        ...
        // Since iOS 15 barTintColor is ignored in favour of UITabBarAppearance,
        // and an unset scrollEdgeAppearance leaves the bar transparent, so it
        // picked up whatever was behind it and changed colour from tab to tab.
    ...
    fileprivate func applyNavigationBarApperance() {
        ...
        if let navTitleColor = self.value(aClass: aClass, property: .textColor) as? UIColor {
            navTitleAttributes[NSAttributedString.Key.foregroundColor] = navTitleColor
        }
        if let navTitleFont = self.value(aClass: aClass, property:. textFont) as? UIFont {
            navTitleAttributes[NSAttributedString.Key.font] = navTitleFont
        }
        navigationBar.titleTextAttributes = navTitleAttributes

        // Same iOS 15 change as the tab bar: barTintColor, the background image
        // and the title attributes above are all superseded by
        // UINavigationBarAppearance. Mirror what was just configured, including
        // the themed removal of the bottom shadow.
        let appearance = UINavigationBarAppearance()
        if let barTint = navigationBar.barTintColor {
            appearance.configureWithOpaqueBackground()
            appearance.backgroundColor = barTint
        } else {
            appearance.configureWithDefaultBackground()
        ...
        appearance.titleTextAttributes = navTitleAttributes
