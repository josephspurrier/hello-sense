...
@objcMembers class OtherGenderOptionsPresenter: HEMListPresenter {
    ...
    override func bind(with navItem: UINavigationItem) {
        ...
        searchImage = searchImage?.withRenderingMode(UIImage.RenderingMode.alwaysTemplate)
        ...
        searchButton.setImage(searchImage, for: UIControl.State.normal)
        searchButton.addTarget(self, action: searchAction, for: UIControl.Event.touchUpInside)
    ...
    @objc fileprivate func showSearch() {
        ...
        self.searchBar!.setImage(searchIcon, for: UISearchBar.Icon.search, state: UIControl.State.normal)
...
extension OtherGenderOptionsPresenter {
    ...
    @objc fileprivate func keyboard(willShow notification: NSNotification) {
        ...
        let rawCurve = (info[UIResponder.keyboardAnimationCurveUserInfoKey] as! NSNumber).intValue << 16
        let curve = UIView.AnimationOptions(rawValue: UInt(rawCurve))
