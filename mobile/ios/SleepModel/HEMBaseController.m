...
- (void)didChangeWithTheme:(Theme *)theme auto:(BOOL)auto_ {
    // applyStyle is otherwise only run from viewDidLoad, so a controller that
    // was already loaded kept the previous theme's background and showed
    // through wherever its view does not cover, such as behind the tab bar.
